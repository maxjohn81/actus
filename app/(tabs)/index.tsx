...
import { View, Text, ActivityIndicator, TouchableOpacity, ImageBackground, Pressable, Image, FlatList, RefreshControl } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { Search, Bookmark, RefreshCw } from 'lucide-react-native';
...
export default function HomeScreen() {
  ...
  const navigueToSearch = () => router.push('/(tabs)/recherche');
    ...
    <SafeAreaView className="flex-1 bg-[#F8F9FF] dark:bg-[#0B0F1A]">
      ...
      <View className="m-3 mt-5">
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            >
              <Text className={`font-medium text-xl ${search === item.key ? 'text-white' : 'text-neutral-700 dark:text-neutral-300'}`}>
                {item.title}
              </Text>
            </Pressable>
          )}
      ...
      {error ? (
        <View className="m-3 flex-1 justify-center items-center">
          <Text className="text-red-500 text-xl mb-2">⚠️ Limite atteinte</Text>
          <Text className="text-gray-600 dark:text-gray-400 text-center mb-6">
            Vous avez atteint la limite de requêtes NewsAPI.{'\n'}
            Revenez dans quelques heures ou passez à un plan payant.
          </Text>
          <TouchableOpacity
            onPress={onRefresh}
            style={{ padding: 4 }}
            className="bg-violet-600 px-8 py-3 rounded-xl flex-row items-center gap-2"
          >
            <RefreshCw size={20} color="white" />
            <Text className="text-white font-semibold px-2" style={{ padding: 4 }}>Réessayer</Text>
          </TouchableOpacity>
        </View>
      ) : (
        <>
          ...
          {currentArticle && (
            <View className="m-3 rounded-xl overflow-hidden" style={{ height: 300 }}>
              <TouchableOpacity activeOpacity={0.9} style={{ height: 300 }}>
                <ImageBackground
                ...
                >
                  <View className="absolute top-12 left-5 bg-violet-600 px-4 py-1.5 rounded-md" style={{ zIndex: 10 }}>
                    <Text className="text-white font-bold text-xl tracking-wider">À LA UNE</Text>
                  </View>

                  <View className="bg-black/75 p-5 pb-10 justify-end" style={{ height: 300 }}>
                    <Text className="text-white text-3xl font-bold leading-7" numberOfLines={3}>
                      {currentArticle.title}
                    </Text>
                    <Text className="text-gray-300 mt-3 text-base">
                      {new Date(currentArticle.publishedAt).toLocaleDateString('fr-FR')} • {getTimeAgo(currentArticle.publishedAt)}
                    </Text>
                  </View>
                </ImageBackground>
              </TouchableOpacity>
          ...
          {/* Liste des articles */}
          <View className='m-3 mt-2 flex flex-row justify-between items-center'>
            <Text className='text-3xl'>Dernières actualités</Text>
              ...
              renderItem={({ item }) => (
                <View className="flex-row justify-between mb-6">
                  <TouchableOpacity
                    ...
                    onPress={() => router.push({
                      pathname: "/article/[id]",
                      params: { id: encodeURIComponent(item.url), article: JSON.stringify(item) }
                    })}
                  >
                    <Image
                  ...
                  <Pressable className="ml-3 p-2" onPress={() => handleToggleFavorite(item)}>
                    <Bookmark
                      size={30}
